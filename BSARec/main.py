...
cfg.add_argument("--attn-dropout-rate", type=float, default=0.5)
...
cfg.add_argument(
    "--alpha", type=float, default=0.7, help="the ratio for frequency domain"
)
...
class BSARec(freerec.models.SeqRecArch):
    ...
    -> stacked BSARec blocks (multi-head self-attention + low-pass frequency filter)
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([BSARecBlock(cfg) for _ in range(cfg.num_blocks)])
