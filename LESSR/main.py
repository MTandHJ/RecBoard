...
cfg.add_argument("--batch-norm", type=bool, default=True)
...
class LESSR(freerec.models.SeqRecArch):
    """
    session items -> build EOP/shortcut/session graphs per session
    -> item embds lookup -> alternating EOPA (on EOP graph) and SGAT
    (on shortcut graph) layers with feature concatenation (K layers)
    -> attention readout (local last-item + global attentive sum)
    -> concat -> BatchNorm -> dropout -> linear projection
    -> dot product with item embds -> CE loss.
    """
    ...
    def __init__(
        ...
        for i in range(cfg.num_layers):
            if i % 2 == 0:
                layer = EOPA(
                    ...
                    activation=nn.PReLU(cfg.embedding_dim),
                )
            else:
                layer = SGAT(
                    input_dim,
        ...
        input_dim += cfg.embedding_dim
        self.batch_norm = nn.BatchNorm1d(input_dim) if cfg.batch_norm else None
        self.feat_drop = nn.Dropout(cfg.dropout_rate)
        self.fc_sr = nn.Linear(input_dim, cfg.embedding_dim, bias=False)
    ...
    def sure_trainpipe(self, maxlen: int, batch_size: int):
        return (
            self.dataset.train()
            .shuffled_roll_seqs_source(minlen=2, keep_at_least_itself=True)
    ...
    def get_multi_graphs(self, seqs: torch.Tensor, masks: torch.Tensor):
        def extract_from_seq(i: int, seq: np.ndarray, mask: np.ndarray):
            ...
            # EOP
            graph_eop = Data(x=x, edge_index=torch.LongTensor([seq[:-1], seq[1:]]))
            graph_eop.nodes = torch.from_numpy(items)

            # Shortcut
            graph_cut = Data(x=x, edge_index=coalesce(graph_eop.edge_index))
        ...
        graph_eop, graph_cut, graph_sess = zip(
            *map(
                extract_from_seq,
                range(len(seqs)),
                seqs.cpu().numpy(),
        ...
        return (
            graph_eop.to(self.device),
            graph_cut.to(self.device),
            graph_sess.to(self.device),
        )
    ...
    def encode(
        ...
        seqs = data[self.ISeq]
        masks = seqs.ne(0)
        graph_eop, graph_cut, graph_sess = self.get_multi_graphs(seqs, masks)
