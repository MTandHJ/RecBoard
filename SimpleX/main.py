...
cfg.add_argument("--gamma", type=float, default=1.0)  # 0., 0.5, 0.1
cfg.add_argument("--margin", type=float, default=0.9)  # 0:1:0.1
...
class AvgConv(MessagePassing):
    """Average convolution layer for neighbor aggregation."""
...
class CosineContrastiveLoss(freerec.criterions.BaseCriterion):
    """Cosine contrastive loss with margin for positive-negative separation."""
...
class SimpleX(freerec.models.GenRecArch):
    """
    item embds -> mean aggregation over user's interacted items -> linear
    -> weighted mix with user embds (gamma) -> L2-normalize -> dropout
    + item embds -> L2-normalize
    -> cosine similarity -> cosine contrastive loss with margin.
    """
    ...
    def __init__(
        ...
        self.aggregator = AvgConv(
            embedding_dim=cfg.embedding_dim, size=(self.User.count, self.Item.count)
        )
    ...
    def aggregate(self, users: torch.Tensor) -> torch.Tensor:
        ...
        return userEmbds * self.gamma + self.aggregator(items, edge_index)[users] * (
            1 - self.gamma
        )

    def encode(self, data: Dict[freerec.data.fields.Field, torch.Tensor]):
        userEmbds = F.normalize(self.aggregate(data[self.User]), dim=-1)  # (B, 1, D)
        itemEmbds = F.normalize(self.Item.embeddings.weight, dim=-1)  # (N, D)
        return self.dropout(userEmbds), itemEmbds

    def fit(self, data: Dict[freerec.data.fields.Field, torch.Tensor]):
        userEmbds, itemEmbds = self.encode(data)
        items = torch.cat((data[self.Item], data[self.INeg]), dim=1)
