...
cfg.add_argument("--embedding-decay", type=float, default=1.0e-5)
...
class DCN(freerec.models.PredRecArch):
    """
    field embds -> flatten -> parallel branches:
      cross network (repeated x0 * W(xi) + bias) -> cross output
      DNN (MLP blocks with ReLU/BN/dropout) -> deep output
    -> concat cross & deep -> FC -> logit -> BCE loss.
    """
