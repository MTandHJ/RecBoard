...
cfg.add_argument(
    "--num-blocks", default=2, type=int, help="number of filter-enhanced blocks"
)
...
class FMLPRec(freerec.models.SeqRecArch):
    """
    item embds + position embds -> LayerNorm -> dropout
    -> stacked filter-enhanced blocks (FFT -> learnable frequency filter -> iFFT + MLP)
    -> last position hidden -> dot product with item embds
    -> BPR/BCE/CE loss.
    """
    ...
    def __init__(
        ...
        self.layerNorm = LayerNorm(cfg.embedding_dim, eps=1.0e-12)
    ...
    def mark_position(self, seqs: torch.Tensor):
        S = seqs.size(1)
        positions = torch.arange(0, S, dtype=torch.long, device=self.device).unsqueeze(
            0
        )
    ...
    def encode(
        ...
        userEmbds = self.itemEncoder(seqs, attnMask, output_all_encoded_layers=True)[
            -1
        ]  # (B, S, D)
