...
from modules import DistSAEncoder, LayerNorm, kl_distance, wasserstein_distance
...
cfg.add_argument("--attn-dropout-rate", type=float, default=0.0)
cfg.add_argument(
    "--distance-metric", type=str, choices=("wasserstein", "kl"), default="wasserstein"
)
cfg.add_argument(
    "--pvn-weight",
    type=float,
    default=0.005,
    help="the weight for postives versus negatives",
)
...
class STOSA(freerec.models.SeqRecArch):
    """
    item mean embds + position mean embds -> layer norm -> dropout -> ELU
    item cov embds + position cov embds -> layer norm -> dropout -> ELU + 1
    -> distributional self-attention encoder (K blocks)
    -> negative Wasserstein/KL distance scoring -> BPR loss + PVN regularization loss.
    """
    ...
    def __init__(
        ...
        self.pos_mean_embds = nn.Embedding(cfg.maxlen, cfg.embedding_dim)
        self.pos_cov_embds = nn.Embedding(cfg.maxlen, cfg.embedding_dim)

        self.embdLN = LayerNorm(cfg.embedding_dim, eps=1e-12)
        ...
        self.register_buffer(
            "attnMask",
            torch.ones(
                (1, 1, cfg.maxlen, cfg.maxlen), dtype=torch.bool
            ).tril(),  # (1, 1, maxlen, maxlen)
        )
    ...
    def sure_trainpipe(self, maxlen: int, batch_size: int):
        return (
            ...
            .lpad_(
                maxlen,
                modified_fields=(self.ISeq, self.IPos, self.INeg),
    ...
    def fit(self, data: Dict[freerec.data.fields.Field, torch.Tensor]):
        ...
        posLogits = dist_func(
            mean_users, cov_users, mean_positives, cov_positives
        ).neg()

        negLogits = dist_func(
            mean_users, cov_users, mean_negatives, cov_negatives
        ).neg()

        pvnLogits = dist_func(
            mean_positives, cov_positives, mean_negatives, cov_negatives
        ).neg()

        rec_loss = self.criterion(posLogits, negLogits)
        pvn_loss = self.pvn_loss(posLogits, pvnLogits)

        return {"rec_loss": rec_loss, "pvn_loss": pvn_loss}
    ...
    def recommend_from_full(self, data: Dict[freerec.data.fields.Field, torch.Tensor]):
        ...
        return dist_func(mean_users, cov_users, mean_items, cov_items).neg()
...
class CoachForSTOSA(freerec.launcher.Coach):
    """Coach for STOSA training."""

    def train_per_epoch(self, epoch: int):
        for data in self.dataloader:
            data = self.dict_to_device(data)
            losses = self.model(data)
            loss = losses["rec_loss"] + losses["pvn_loss"] * self.cfg.pvn_weight
