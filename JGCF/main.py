...
cfg.add_argument(
    "--scaling-factor", type=float, default=3.0, help="hyper-parameter for rescaling"
)
cfg.add_argument(
    "--alpha", type=float, default=1.0, help="hyper-parameter for Jacobi Polynomial"
)
cfg.add_argument(
    "--beta", type=float, default=1.0, help="hyper-parameter for Jacobi Polynomial"
)
...
class JGCF(freerec.models.GenRecArch):
    """
    user/item embds -> concat -> Jacobi polynomial convolution on sym-normalized adj
    -> low-pass output; mid-pass = weighted original - low-pass
    -> concat(low-pass, mid-pass) -> split user/item
    ...
    def encode(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        allEmbds = torch.cat((userEmbds, itemEmbds), dim=0).flatten(1)  # N x D
