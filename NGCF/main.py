...
class NGCFConv(nn.Module):
    """NGCF graph convolution layer with element-wise interaction."""
...
class NGCF(freerec.models.GenRecArch):
    """
    user/item embds -> concat -> NGCF convolution (K layers): each layer
    computes LeakyReLU(W1*(Ax + x) + W2*(Ax * x)) -> L2 normalize -> dropout
    -> concat all layer outputs -> split user/item
    ...
    def __init__(
        ...
        self.convs = nn.ModuleList(
            [
                NGCFConv(cfg.embedding_dim, cfg.embedding_dim, cfg.dropout_rate)
