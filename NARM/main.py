...
class NARM(freerec.models.SeqRecArch):
    """
    item embds -> dropout -> GRU (multi-layer)
    -> global: last hidden state; local: bilinear attention
    (linear(GRU out) + linear(last hidden)) -> sigmoid -> weighted sum
    -> concat(local, global) -> dropout -> linear projection
    -> dot product with item embds -> BCE loss.
    """
    ...
    def __init__(
        ...
        self.gru = nn.GRU(
            ...
            bias=False,
            batch_first=True,
            dropout=cfg.hidden_dropout_rate,
        )
