...
cfg.add_argument(
    "--weight4mAdj", type=float, default=0.1, help="weight for fusing vAdj and tAd"
)
cfg.add_argument(
    "--weight4modality", type=float, default=1.0e-3, help="weight for modality BPR loss"
)
cfg.add_argument(
    "--sampling-ratio", type=float, default=0.2, help="sampling ratio for U-I graph"
)
...
assert cfg.sampling_ratio >= 0.0, f"Invalid sampling ratio {cfg.sampling_ratio}"


class IISide(nn.Module):
    """Item-item side information module with modality-aware kNN graph."""
    ...
    def get_knn_graph(self, features: torch.Tensor):
        ...
        edge_index, _ = freerec.graph.get_knn_graph(sim, cfg.knn_k, symmetric=False)
...
class FREEDOM(freerec.models.GenRecArch):
    """
    user/item embds -> LightGCN propagation on sampled user-item graph
    item embds -> GCN propagation on modality-fused item-item kNN graph -> add to item feats
    visual/textual embds -> linear projectors -> modality item feats
    -> dot product -> BPR loss (ID + modality) + L2 regularization.
    """
    ...
    def __init__(
        ...
        self.sampling_probs = self.normalize_graph(self.interactions)
    ...
    def normalize_graph(self, edge_index: torch.Tensor):
        ...
        row_sum = 1.0e-7 + scatter(edge_weight, row, dim=0, dim_size=self.User.count)
        col_sum = 1.0e-7 + scatter(edge_weight, col, dim=0, dim_size=self.Item.count)
    ...
    def encode(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        iiEmbds, vFeats, aFeats = self.iiSide(itemEmbds)
        userFeats, itemFeats = torch.split(avgFeats, (self.User.count, self.Item.count))

        return userFeats, itemFeats + iiEmbds, vFeats, aFeats
    ...
    def fit(self, data: Dict[freerec.data.fields.Field, torch.Tensor]):
        ...
        return {"rec_loss": rec_loss, "mod_loss": mod_loss, "emb_loss": emb_loss}
...
class CoachForFREEDOM(freerec.launcher.Coach):
    ...
    def train_per_epoch(self, epoch: int):
        ...
        for data in self.dataloader:
            ...
            loss = (
                losses["rec_loss"] + self.cfg.weight4modality * losses["mod_loss"]
            )  # + self.cfg.weight_decay * losses["emb_loss"]
