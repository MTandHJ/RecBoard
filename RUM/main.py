...
class RUM(freerec.models.SeqRecArch):
    """
    item seq embds + candidate item embds -> attention-weighted memory read
    -> scale and add to user embds
    -> dropout(element-wise product with item embds) -> sum -> BPR loss.
    """
    ...
    def __init__(
        ...
        self.Item.add_module(
            "embeddings",
            nn.Embedding(self.Item.count + self.NUM_PADS, cfg.embedding_dim),
        )
    ...
    def read_memory(
        ...
        sim_matrix.masked_fill_(padding_mask.unsqueeze(-1).expand_as(sim_matrix), -1e23)
    ...
    def fit(self, data: Dict[freerec.data.fields.Field, torch.Tensor]):
        ...
        rec_loss = self.criterion(scores[:, :1], scores[:, 1:])
