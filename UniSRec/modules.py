...
class PWLayer(nn.Module):
    ...
    def __init__(self, input_size: int, output_size: int, dropout_rate: float = 0.):
        super().__init__()
...
class MoEAdaptorLayer(nn.Module):
    ...
    def __init__(self, n_exps: int, input_size: int, output_size: int, dropout_rate: float = 0., noise: bool = True):
        super().__init__()

        self.n_exps = n_exps
        self.noisy_gating = noise

        self.experts = nn.ModuleList([PWLayer(input_size, output_size, dropout_rate) for i in range(n_exps)])
    ...
    def noisy_top_k_gating(self, x: torch.Tensor, train: bool, noise_epsilon: float = 1.e-2):
