...
cfg.add_argument("--ssl-drop-rate", type=float, default=0.1)
...
class SGL(freerec.models.GenRecArch):
    """
    user/item embds -> normalized adjacency propagation with layer-wise mean (K layers)
    -> dot product -> BPR loss + L2 embd regularization
    + two augmented subgraphs (ND/ED/RW) -> L2-normalize
    -> cross-view InfoNCE SSL loss.
    """
    ...
    def __init__(self, dataset: freerec.data.datasets.RecDataSet) -> None:
        ...
        self.register_buffer("Adj", self.dataset.train().to_normalized_adj())
    ...
    def sample_subgraph(self):
        ...
        if cfg.aug_type == "nd":
            user_weight = (
                torch.rand(self.User.count, device=self.device) > cfg.ssl_drop_rate
            ).float()
            item_weight = (
                torch.rand(self.Item.count, device=self.device) > cfg.ssl_drop_rate
            ).float()
            edge_weight = user_weight[row] * item_weight[col]
        elif cfg.aug_type in ("ed", "rw"):
            edge_weight = (
                torch.rand(len(row), device=self.device) > cfg.ssl_drop_rate
            ).float()
        else:
    ...
    def fit(self, data: Dict[freerec.data.fields.Field, torch.Tensor]):
        ...
        ssl_user_logits = torch.einsum("MKD,NKD->MN", userEmbds, userEmbds_).div(
            cfg.temperature
        )
        ssl_item_logits = torch.einsum("MKD,NKD->MN", itemEmbds, itemEmbds_).div(
            cfg.temperature
        )
        targets = torch.arange(
            ssl_user_logits.size(0), device=self.device, dtype=torch.long
        )
        ssl_loss = freerec.criterions.cross_entropy_with_logits(
            ssl_user_logits, targets
        ) + freerec.criterions.cross_entropy_with_logits(ssl_item_logits, targets)

        return {"rec_loss": rec_loss, "emb_loss": emb_loss, "ssl_loss": ssl_loss}
    ...
    def train_per_epoch(self, epoch: int):
        ...
        for data in self.dataloader:
            ...
            loss = (
                ...
                + self.cfg.ssl_weight * losses["ssl_loss"]
            )
