...
from modules import CrossAttTransformerLayer, TransformerLayer
...
cfg.add_argument(
    "--sem-id-ckpt", type=str, default=None, help="checkpoint file of 'sem_ids'"
)
cfg.add_argument(
    "--tfiles", type=str, default=None, help="checkpoint file of textual features"
)
...
class SemIDEmbedding(nn.Embedding):
    """Semantic ID embedding for item representation."""
    ...
    def __init__(
        ...
        num_embeddings += 2
        sem_ids = sem_ids + 2
        sem_ids = F.pad(sem_ids, (0, 0, 1, 0), value=masking_token)
        sem_ids = F.pad(sem_ids, (0, 0, 1, 0), value=padding_token)

        super().__init__(num_embeddings, embedding_dim, padding_token)

        self.register_buffer("sem_ids", sem_ids)


class QFormer(nn.Module):
    """Query-based cross-attention transformer for feature fusion."""
...
class Transformer(nn.Module):
    """Causal transformer encoder for sequence modeling."""
...
class CCFRec(freerec.models.SeqRecArch):
    """
    semantic-ID embds (multi-codebook) -> QFormer cross-attention with PCA-reduced text features
    -> mean pool (+ residual) -> + position embds -> LayerNorm -> dropout
    -> causal Transformer encoder -> last position hidden -> L2 normalize
    -> dot product with L2-normalized item embds / tau -> CE loss
    + masked semantic-ID prediction -> dot product / tau -> CE loss
    + contrastive loss between clean and masked user representations.
    """
    ...
    def __init__(
        ...
        for tfile in cfg.tfiles.split(","):
            num_fields += 1
            features.append(
                PCA(n_components=self.embedding_dim, whiten=True).fit_transform(
                    freerec.utils.import_pickle(
                        os.path.join(self.dataset.path, tfile)
                    )  # (N, D)
                )
            )
        ...
        self.register_buffer(
            "features", rearrange(features, "N (K D) -> N K D", K=num_fields)
        )
    ...
    def sure_trainpipe(self, maxlen: int, batch_size: int):
        return (
            ...
            .seq_train_sampling_neg_(num_negatives=cfg.num_negs)
            .add_(
    ...
    def extend_attention_mask(self, seqs: torch.Tensor):
        ...
        attention_mask = repeat(attention_mask, "B 1 1 S -> B 1 L S", L=seqs.size(1))
    ...
    def encode_item(
        ...
        return {
            "unpooled": itemEmbds.view(
                *items.shape, itemEmbds.size(-2), itemEmbds.size(-1)
            ),  # (*, K, D)
            "pooled": (itemEmbds + query)
            .mean(dim=1)
            .view(*items.shape, itemEmbds.size(-1)),  # (*, D)
        }

    def encode_user(self, seqs: torch.Tensor, seqEmbds: torch.Tensor) -> torch.Tensor:
    ...
    def mask_and_replace(self, sem_ids: torch.Tensor, p: float):
        ...
        masked_seqs = torch.where(
            ...
            torch.randint(
                0, self.Item.embeddings.weight.size(0), size=sem_ids.size()
            ).to(self.device),
            sem_ids,
        )
    ...
    def fit(
        ...
        itemEmbds = self.encode_item(torch.cat((positives, negatives), dim=-1))[
            "pooled"
        ]  # (B, K + 1, D)
        ...
        masked_sem_ids, labels, masks = self.mask_and_replace(
            self.Item.embeddings.sem_ids[seqs], p=cfg.mask_ratio
        )
        ...
        cl_loss = (
            self.criterion(logits, labels) + self.criterion(logits.T, labels)
        ) / 2

        return {"rec_loss": rec_loss, "mlm_loss": mlm_loss, "cl_loss": cl_loss}
    ...
    def reset_ranking_buffers(self):
        ...
        for items_ in torch.split(items, LOCAL_BATCH_SIZE, dim=0):
            itemEmbds.append(self.encode_item(items_)["pooled"])
...
class CoachForCCFRec(freerec.launcher.Coach):
    ...
    def train_per_epoch(self, epoch: int):
        for data in self.dataloader:
            ...
            loss = (
                losses["rec_loss"]
                + cfg.weight4mlm * losses["mlm_loss"]
                + cfg.weight4cl * losses["cl_loss"]
            )
