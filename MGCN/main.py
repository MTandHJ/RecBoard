...
from freerec.data.tags import ID, ITEM, USER
...
cfg.add_argument(
    "--num-layers", type=int, default=2, help="the number of layers for U-I graph"
)

cfg.add_argument("--knn-k", type=int, default=10, help="top-k knn graph")
cfg.add_argument(
    "--weight4cl", type=float, default=0.01, help="weight for contrastive loss"
)
cfg.add_argument(
    "--temperature", type=float, default=0.2, help="temperature for contrastive loss"
)
...
class Purifier(nn.Module):
    """Modality purifier with gated projection."""
    ...
    def forward(self, mEmbds: torch.Tensor, iEmbds: torch.Tensor):
        ...
        return iEmbds.mul(self.gate(mEmbds))


class MGCN(freerec.models.GenRecArch):
    """
    UI view: user/item embds -> concat -> LightGCN propagation (K layers)
    -> layer-wise mean pooling;
    II view: per-modality features -> gated purifier (linear + sigmoid gate * item embds)
    -> kNN adj propagation -> R^T to get user embds -> stack modalities;
    fusion: modality embds -> attention query -> softmax -> shared + self components
    -> preference-gated aggregation -> add UI view
    -> split user/item -> dot product -> BPR loss + InfoNCE loss + L2 regularization.
    """
    ...
    def __init__(
        ...
        self.register_buffer("Adj", dataset.train().to_normalized_adj())

        # U-I Interaction matrix for I-I View
        g = dataset.train().to_bigraph((USER, ID), (ITEM, ID), edge_type="U2I")
        ...
        if cfg.vfile:
            vAdj, vFeats = self.load_feats(dataset.path, cfg.vfile)
            self.register_buffer("vAdj", vAdj)
            self.vFeats = vFeats
            self.vPurifier = Purifier(self.vFeats.weight.size(1), cfg.embedding_dim)

        if cfg.tfile:
            tAdj, tFeats = self.load_feats(dataset.path, cfg.tfile)
            self.register_buffer("tAdj", tAdj)
            self.tFeats = tFeats
            self.tPurifier = Purifier(self.tFeats.weight.size(1), cfg.embedding_dim)

        if cfg.afile:
            aAdj, aFeats = self.load_feats(dataset.path, cfg.afile)
            self.register_buffer("aAdj", aAdj)
            self.aFeats = aFeats
            self.aPurifier = Purifier(self.aFeats.weight.size(1), cfg.embedding_dim)
        ...
        self.preference_gates = nn.ModuleList(
            [
                nn.Sequential(
                    nn.Linear(cfg.embedding_dim, cfg.embedding_dim), nn.Sigmoid()
                )
    ...
    def item_item_encode(
        self, mEmbds: torch.Tensor, mAdj: torch.Tensor, R: torch.Tensor
    ):
        r"""
        ...
        mEmbds_u = R @ mEmbds_i
        return torch.cat((mEmbds_u, mEmbds_i), dim=0)
    ...
    def encode(self):
        ...
        if cfg.vfile:
            ...
            vEmbds = self.item_item_encode(vEmbds, self.vAdj, self.R)
            mEmbds.append(vEmbds)
        if cfg.tfile:
            tEmbds = self.tPurifier(self.tFeats.weight, itemEmbds)
            tEmbds = self.item_item_encode(tEmbds, self.tAdj, self.R)
            mEmbds.append(tEmbds)
        if cfg.afile:
            aEmbds = self.aPurifier(self.aFeats.weight, itemEmbds)
            aEmbds = self.item_item_encode(aEmbds, self.aAdj, self.R)
        ...
        preferences = torch.stack(
            [gate(iEmbds) for gate in self.preference_gates], dim=1
        )  # (N, X, D)

        mEmbds = self.fuse(mEmbds, preferences)
        finalEmbds = iEmbds + mEmbds
        userEmbds, itemEmbds = torch.split(
            finalEmbds, (self.User.count, self.Item.count), dim=0
        )
    ...
    def fit(self, data: Dict[freerec.data.fields.Field, torch.Tensor]):
        ...
        mEmbds_u, mEmbds_i = torch.split(
            mEmbds, (self.User.count, self.Item.count), dim=0
        )
        iEmbds_u, iEmbds_i = torch.split(
            iEmbds, (self.User.count, self.Item.count), dim=0
        )
        users, items = users.flatten(), items[:, 0]
        cl_loss = self.InfoNCE(
            mEmbds_u[users], iEmbds_u[users], cfg.temperature
        ) + self.InfoNCE(mEmbds_i[items], iEmbds_i[items], cfg.temperature)
        ...
        return {"rec_loss": rec_loss, "cl_loss": cl_loss, "emb_loss": emb_loss}
...
class CoachForMGCN(freerec.launcher.Coach):
    """Coach for MGCN training."""

    def train_per_epoch(self, epoch: int):
        for data in self.dataloader:
            data = self.dict_to_device(data)
            losses = self.model.fit(data)
            loss = (
                losses["rec_loss"]
                + losses["emb_loss"] * self.cfg.weight_decay
                + losses["cl_loss"] * self.cfg.weight4cl
            )
