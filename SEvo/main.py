...
from optims import AdamSEvo, AdamWSEvo, SGDSEvo, Smoother, get_graph

freerec.declare(version="0.8.7")
...
cfg.add_argument(
    "--aggr", type=str, choices=("neumann", "iterative"), default="neumann"
)
cfg.add_argument(
    "--L", type=int, default=3, help="the number of layers for approximation"
)
cfg.add_argument("--beta3", type=float, default=0.9, help="the `beta` in Eq. (6)")
cfg.add_argument(
    "--H",
    type=int,
    default=1,
    help="the maximum walk length allowing for a pair of neighbors",
)
cfg.add_argument(
    "--maxlen4graph",
    type=int,
    default=50,
    help="only the last `maxlen` items in a sequence will be used for construciton",
)
...
class PointWiseFeedForward(nn.Module):
    """Point-wise feed-forward network with 1D convolutions."""
...
class SASRec(freerec.models.SeqRecArch):
    """
    item embds (graph-smoothed via SEvo optimizer) * sqrt(D) + position embds -> dropout
    -> layer norm + causal multi-head self-attention (residual)
    -> layer norm + point-wise FFN with Conv1d (residual) (K blocks)
    -> layer norm -> dot product -> BCE/BPR/CE loss.
    """
    ...
    def __init__(
        ...
        for _ in range(cfg.num_blocks):
            self.attnLNs.append(nn.LayerNorm(cfg.embedding_dim, eps=1e-8))
            ...
            self.fwdLNs.append(nn.LayerNorm(cfg.embedding_dim, eps=1e-8))

            self.fwdLayers.append(
                PointWiseFeedForward(cfg.embedding_dim, cfg.dropout_rate)
            )
        ...
        self.register_buffer(
            "attnMask",
            torch.ones((cfg.maxlen, cfg.maxlen), dtype=torch.bool).triu(diagonal=1),
        )
    ...
    def marked_params(self):
        smooth_params = [self.Item.embeddings.weight]
        recorded = [id(param) for param in smooth_params]
        regular_params = [
        ...
        return [
            {"params": regular_params},
            {
                ...
                "smoother": Smoother(
                    Adj=get_graph(cfg, self.dataset, self.NUM_PADS).to(cfg.device),
                    beta=cfg.beta3,
                    L=cfg.L,
                    aggr=cfg.aggr,
                ),
    ...
    def encode(
        ...
        seqs.masked_fill_(padding_mask, 0.0)
...
class CoachForSASRec(freerec.launcher.Coach):
    """Coach for SASRec with SEvo training."""

    def set_optimizer(self):
        marked_params = self.model.marked_params()
        ...
        elif self.cfg.optimizer.lower() == "sgdsevo":
            self.optimizer = SGDSEvo(
                marked_params,
                lr=cfg.lr,
