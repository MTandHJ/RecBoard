...
class RQVAE(freerec.models.RecSysArch):
    def __init__(self, dataset: freerec.data.datasets.RecDataSet) -> None:
        ...
        self.encoder = DIGERIDEncoder(
            in_dim=self.Item.embeddings.weight.size(1),
        ...
        self.decoder = DIGERIDDecoder(
            out_dim=self.Item.embeddings.weight.size(1),
            hidden_dims=cfg.hidden_dims[::-1],
            codebook_dim=cfg.codebook_dim,
            dropout_rate=cfg.dropout_rate,
        )
...
class CoachForRQVAE(freerec.launcher.Coach):
    ...
    def save_tokenizer(self):
        state_dict = {
            "encoder": self.get_res_sys_arch().encoder.state_dict(),
            "quantizer": self.get_res_sys_arch().quantizer.state_dict(),
        }
        return state_dict

    def save_best(self):
        if freerec.ddp.is_main_process():
            torch.save(self.save_tokenizer(), os.path.join(self.cfg.LOG_PATH, "tokenizer.pt"))
