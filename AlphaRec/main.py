...
cfg.add_argument("--num-layers", type=int, default=2)
...
cfg.add_argument("--num-negs", type=int, default=256)
...
class AlphaRec(freerec.models.GenRecArch):
    """
    frozen text embds -> projector
    -> LightGCN propagation -> L2 normalize
    -> dot product / tau -> CE loss.
    """
    ...
    def __init__(
        ...
        self.Item.add_module(
            ...
            nn.Embedding.from_pretrained(
                freerec.utils.import_pickle(os.path.join(dataset.path, cfg.tfile)),
                freeze=True,
            ),
        )

        # R
        R = dataset.train().to_bigraph((USER, ID), (ITEM, ID), edge_type="U2I")
        ...
        self.User.add_module(
            "embeddings",
            nn.Embedding.from_pretrained(R @ self.Item.embeddings.weight, freeze=True),
        )
    ...
    def sure_trainpipe(self, batch_size: int):
        return (
            ...
            .gen_train_sampling_neg_(num_negatives=cfg.num_negs, unseen_only=True)
            .batch_(batch_size)
            .tensor_()
        )

    def encode(self) -> Tuple[torch.Tensor, torch.Tensor]:
        userEmbds = self.projector(self.User.embeddings.weight)
        itemEmbds = self.projector(self.Item.embeddings.weight)
    ...
    def fit(self, data: Dict[freerec.data.fields.Field, torch.Tensor]):
        ...
        labels = torch.zeros(
            (userFeats.size(0),), device=userFeats.device, dtype=torch.long
        )

        rec_loss = self.criterion(
            torch.einsum("BKD,BKD->BK", userFeats, itemFeats) / cfg.tau, labels
        )
