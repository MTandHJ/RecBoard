...
cfg.add_argument("--lora-dropout-rate", type=float, default=0.05)
...
class E4SRec(freerec.models.SeqRecArch):
    """
    frozen item ID embds -> linear adaptor
    -> concat(instruction embds, adapted seq embds, response embds)
    -> 8-bit LLaMA with LoRA -> last hidden state
    -> linear output projection -> CE loss.
    """
    ...
    def __init__(
        ...
        self.tokenizer = LlamaTokenizer.from_pretrained(
            cfg.saved_model, use_fast=False, local_files_only=True
        )
        ...
        instruction = prompts["prompt_input"].format(
            instruction=(
                "Given the user’s purchase history, "
                "predict next possible item to be purchased."
            )
        )
        response = prompts["response_split"]

        instruct_ids, instruct_mask = self.tokenizer(
            instruction, padding=False, return_tensors="pt", add_special_tokens=False
        ).values()
        ...
        feats = freerec.utils.import_pickle(f"{cfg.dataset}_id_embeddings.pkl")

        self.Item.add_module(
            "embeddings",
            nn.Embedding.from_pretrained(
                feats, freeze=True, padding_idx=self.PADDING_VALUE
            ),
        )

        self.adaptor = nn.Linear(feats.size(1), self.model.config.hidden_size)
        self.output_proj = nn.Linear(
            self.model.config.hidden_size, self.Item.count + self.NUM_PADS, bias=False
        )
    ...
    def encode(
        ...
        attnMask = torch.cat(
            (instrutMask, seqs.not_equal(self.PADDING_VALUE), responseMask), dim=1
        )

        out = self.model(
            inputs_embeds=inputs, attention_mask=attnMask, return_dict=True
        )
...
class CoachForE4SRec(freerec.launcher.Coach):
    ...
    def load(self, path, filename=None, best: bool = False):
        filename = self.cfg.SAVED_FILENAME if filename is None else filename
        ckpt = torch.load(
            os.path.join(path, filename), map_location=self.device, weights_only=True
        )
        ...
        self.get_res_sys_arch().model.load_adapter(
            os.path.join(self.cfg.LOG_PATH, f"LoRA-{suffix}"), adapter_name="default"
        )
    ...
    def load_best(self) -> None:
        freerec.utils.infoLogger(
            "[Coach] >>> Load best model "
            f"@Epoch: {self._best_epoch} ({self._best_step}) "
        )
    ...
    def set_other(self):
        freerec.utils.mkdirs(
            os.path.join(self.cfg.LOG_PATH, "LoRA-best"),
            os.path.join(self.cfg.LOG_PATH, "LoRA-last"),
        )
    ...
    def set_lr_scheduler(self):
        steps_per_epoch = math.ceil(
            sum([1 for _ in self.trainloader]) // cfg.gradient_accumulation_steps
        )
...
def main():
    ...
    try:
        ...
        dataset = freerec.data.datasets.NextItemRecDataSet(
    ...
    validpipe = model.sure_validpipe(
        maxlen=cfg.maxlen, batch_size=cfg.batch_size, ranking=cfg.ranking
    )
