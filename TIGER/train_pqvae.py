...
from quantizer import ProductQuantizer
...
cfg.add_argument("--codebook-dim", type=int, default=32, help="dimension of each product codebook")
cfg.add_argument(
...
class PQVAE(freerec.models.RecSysArch):
    def __init__(self, dataset: freerec.data.datasets.RecDataSet) -> None:
        ...
        codebook_size = cfg.codebook_dim * cfg.num_codebooks
        dims = [self.Item.embeddings.weight.size(1)] + cfg.hidden_dims + [codebook_size]
        ACT = nn.SiLU
    ...
    def encode(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        z = self.encoder(x)  # (B, #codebooks * D)
        return z
...
class CoachForPQVAE(freerec.launcher.Coach):
