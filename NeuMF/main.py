...
cfg.add_argument("--hidden-dims", type=str, default="64,32,16,8")
...
class NeuMF(freerec.models.GenRecArch):
    """
    user/item embds (MLP branch) -> concat -> linear + ReLU layers (K layers)
    user/item embds (MF branch) -> element-wise product
    -> concat MLP and MF features -> FC layer -> BCE loss.
    """
    ...
    def __init__(
        ...
        self.User.add_module(
            "embeddings4mlp", nn.Embedding(self.User.count, cfg.embedding_dim)
        )

        self.User.add_module(
            "embeddings4mf", nn.Embedding(self.User.count, cfg.embedding_dim)
        )

        self.Item.add_module(
            "embeddings4mlp", nn.Embedding(self.Item.count, cfg.embedding_dim)
        )

        self.Item.add_module(
            "embeddings4mf", nn.Embedding(self.Item.count, cfg.embedding_dim)
        )

        hidden_dims = list(map(int, cfg.hidden_dims.split(",")))
        hidden_dims = [cfg.embedding_dim * 2] + hidden_dims
        ...
        self.fc = nn.Linear(hidden_dims[-1] + cfg.embedding_dim, 1)
    ...
    def encode(self):
        return (
            self.User.embeddings4mlp.weight,
    ...
    def recommend_from_full(self, data: Dict[freerec.data.fields.Field, torch.Tensor]):
        ...
        items = torch.tensor(
            range(self.Item.count), dtype=torch.long, device=self.device
        ).unsqueeze(0)
